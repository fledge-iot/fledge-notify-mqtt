/*
 * Fledge "MQTT" notification plugin.
 *
 * Copyright (c) 2020 Dianomic Systems
 *
 * Released under the Apache 2.0 Licence
 *
 * Author: Mark Riddoch
 */

#include <string>
#include <plugin_api.h>
#include <config_category.h>
#include <logger.h>
#include <mqtt.h>
#include <version.h>

#define PLUGIN_NAME "mqtt"

static const char *default_config = QUOTE({
		"plugin" : { 
			"description" : "MQTT notification plugin - publish an MQTT message",
                       	"type" : "string",
			"default" : PLUGIN_NAME,
			"readonly" : "true"
		       	},
		 "enable": {
		 	"description": "A switch that can be used to enable or disable execution of the notification plugin.",
			"type": "boolean",
			"displayName" : "Enabled",
			"default": "false"
			},
		"broker" : {
			"description" : "The MQTT broker URL",
			"type" : "string",
			"default" : "tcp://mqtt.local:1883",
			"displayName" : "MQTT Broker",
			"order" : "1"
			},
		"topic" : {
			"description" : "MQTT topic to which to publish message",
			"type" : "string",
			"default" : "Fledge",
			"order" : "2",
		       	"displayName" : "MQTT Topic"
			},
		"trigger_payload" : {
			"description" : "Payload to send when the notification triggers",
			"type" : "string",
			"default" : "Triggered",
			"order" : "3",
			"displayName" : "Trigger Payload"
			},
		"clear_payload" : {
			"description" : "Payload to send when the notification clears",
			"type" : "string",
			"default" : "Cleared",
			"order" : "4",
			"displayName" : "Clear Payload"
			}
		});

using namespace std;

/**
 * The Notification plugin interface
 */
extern "C" {

/**
 * The plugin information structure
 */
static PLUGIN_INFORMATION info = {
        PLUGIN_NAME,              // Name
        VERSION,                  // Version
        0,                        // Flags
        PLUGIN_TYPE_NOTIFICATION_DELIVERY,       // Type
        "1.0.0",                  // Interface version
        default_config	          // Default plugin configuration
};

/**
 * Return the information about this plugin
 */
PLUGIN_INFORMATION *plugin_info()
{
	return &info;
}


/**
 * Initialise the plugin, called to get the plugin handle and setup the
 * plugin configuration
 *
 * @param config	The configuration category for the plugin
 * @return		An opaque handle that is used in all subsequent calls to the plugin
 */
PLUGIN_HANDLE plugin_init(ConfigCategory* config)
{
	MQTT *mqtt = new MQTT(config);
	
	return (PLUGIN_HANDLE)mqtt;
}

/**
 * Deliver received notification data
 *
 * @param handle		The plugin handle returned from plugin_init
 * @param deliveryName		The delivery category name
 * @param notificationName	The notification name
 * @param triggerReason		The trigger reason for notification
 * @param message		The message from notification
 */
bool plugin_deliver(PLUGIN_HANDLE handle,
                    const std::string& deliveryName,
                    const std::string& notificationName,
                    const std::string& triggerReason,
                    const std::string& message)
{
	Logger::getLogger()->info("MQTT notification plugin_deliver(): deliveryName=%s, notificationName=%s, triggerReason=%s, message=%s",
							deliveryName.c_str(), notificationName.c_str(), triggerReason.c_str(), message.c_str());
	MQTT *mqtt = (MQTT *)handle;
	return mqtt->notify(notificationName, triggerReason, message);
}

/**
 * Reconfigure the plugin
 */
void plugin_reconfigure(PLUGIN_HANDLE *handle, string& newConfig)
{
	Logger::getLogger()->info("MQTT notification plugin: plugin_reconfigure()");
	MQTT *mqtt = (MQTT *)handle;
	
	mqtt->reconfigure(newConfig);
	return;
}

/**
 * Call the shutdown method in the plugin
 */
void plugin_shutdown(PLUGIN_HANDLE *handle)
{
	MQTT *mqtt = (MQTT *)handle;
	delete mqtt;
}

// End of extern "C"
};

